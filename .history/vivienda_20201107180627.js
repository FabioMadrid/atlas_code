...
//-----------Funcion para la clase Chanchuyo de fake accordion ---//
var header = document.getElementById("caracteristicas");
var btns = header.getElementsByClassName("fake-accordion");
for (var i = 0; i < btns.length; i++) {
  btns[i].addEventListener("click", function() {
  var current = document.getElementsByClassName("active");
  if (current.length > 0) { 
    current[0].className = current[0].className.replace(" active-fake-accordion", "");
  }
  this.className += " active-fake-accordion";
  });
}